...
int _tmain(int argc, LPTSTR argv[])
{
	...
	for (i = 0; i < nRec; i++) {
		iR = randx();
		/* 62 characters, CR, LF */
		_stprintf_s(buffer, sizeof(buffer) / TSIZE, _T("%08x. Record Number: %08d.abcdefghijklmnopqrstuvwxyz x"), iR, i);
		...
		fwrite(buffer, 64, TSIZE, fp); /* 64 characters, including CR, LF, but not EOL */
	}
